...
| `RASPUTIN_BMC_HOST` | compute | `1` marks this node as driving a BMC bus over its own serial port — only for a rack manager on a [BitScope blade rack](/docs/bmc/). It suppresses the login prompt and kernel messages on that port, because there the port is a command channel and stray output is live power traffic. Takes one extra reboot during first boot. Wrong on any other node, and **not** how you configure a Turing Pi — see [BMC](/docs/bmc/). |
